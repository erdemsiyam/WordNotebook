...
public interface IConfuseDAO {
    ...
    List<Confuse> getConfusesWordsByFirstId(Long id);
    @Query("SELECT * FROM Confuse WHERE wrong_word_id=:id")
    List<Confuse> getConfusesWordsBySecondId(Long id);
    @Query("SELECT * FROM Confuse WHERE word_id=:id AND wrong_word_id=:target_id")
    ...
    void deleteConfusesByWordId(Long id);
    @Query("DELETE FROM Confuse")
    void deleteAllConfuse();
}
